...
from models.vae_rna import InfoVAE  # Adjust based on your actual class name
...
_, _, test_idxs = cell_type_split_dataset(atac, annot=True, cell_col='cell_type', cluster_col='leiden', test_ratio=0.1, val_ratio=0.2, seed=19193)
...
model = InfoVAE(input_size=input_size, latent_size=128, lr=0, wd=0, mode="", device=model_params["device"])
state_dict = torch.load("/workspace/runs/atac_vae_run_1/2026-03-15 15:56:11.264790_vae_model_weights.pth")
...
cell_types = atac.obs["cell_type"].values[
    np.where(atac.obs_names.isin(test_idxs))[0]
]
...
adata_latent = sc.AnnData(latent_coords, obs=atac.obs.loc[test_idxs])
